...
function weekPlanData(){
 ...
 const activeCars=cityFilteredCars().filter(c=>(c.driverUserId||c.driverName||c.tenant));
...
function desktopExpenseRows(mode){
 ...
 return (db.expenses||[]).filter(x=>{
  ...
  const d=x.date?new Date(x.date+"T12:00:00"):null;return x.status==="planned"&&d&&d<=to
 })
}
function openDesktopKpiDetails(index){
 ...
 if(index===0){title="Прибыль месяца";subtitle=monthLabel;const accrued=currentMonthAccruedRows();rows=accrued.map(x=>{const p=x.payment;return{name:`${model(car(p.carId)||{}).brand||"Автомобиль"} ${model(car(p.carId)||{}).model||""}`.trim(),meta:`${p.from||p.date||""}${p.to?` — ${p.to}`:""} · ${x.overlapDays}/${x.totalDays} дн.`,amount:x.amount,action:p.id?`payment:${p.id}`:""}});total=rows.reduce((s,x)=>s+x.amount,0);summary="Только часть начислений, которая относится к текущему календарному месяцу"}
 if(index===1){title="Плановый приход недели";subtitle=`${week.from} — ${week.to}`;rows=cityFilteredCars().filter(c=>(c.driverUserId||c.driverName||c.tenant)).map(c=>({name:`${model(c).brand} ${model(c).model} · ${c.plate}`,meta:c.driverName||c.tenant||"Автомобиль с водителем",amount:Number(c.weeklyRent||0),action:`car:${c.id}`}));total=rows.reduce((s,x)=>s+x.amount,0);summary="Считаются все автомобили с назначенным водителем, даже если машине требуется сервис"}
 if(index===2||index===3){const mode=index===2?"plan":"paid";title=index===2?"План расходов недели":"Оплаченные расходы";subtitle=index===2?`${week.from} — ${week.to}`:monthLabel;rows=desktopExpenseRows(mode).map(x=>{const overdue=index===2&&x.date&&new Date(x.date+"T12:00:00")<startOfWeek();return{name:x.title||x.name||x.category||"Расход",meta:`${car(x.carId)?.plate||""}${x.date?` · ${x.date}`:""}${overdue?" · Просрочен":""}`,amount:Number(x.amount||0),action:`expense:${x.id}`}});total=rows.reduce((s,x)=>s+x.amount,0);summary=index===2?"Неоплаченные расходы текущей недели + просроченные с предыдущих недель":"Только фактически оплаченные расходы текущего месяца"}
 if(index===4){const received=currentMonthCashReceived(),paid=desktopExpenseRows("paid").reduce((sum,x)=>sum+Number(x.amount||0),0);title="Фактический результат";subtitle=monthLabel;total=received-paid;summary=`Получено ${money(received)} − оплачено расходов ${money(paid)} = ${money(total)}`;rows=[{name:"Фактически получено",meta:"Текущий месяц · открыть оплаты",amount:received,action:"page:paymentsPage"},{name:"Оплаченные расходы",meta:"Текущий месяц · открыть расходы",amount:-paid,action:"page:expensesPage"}]}
 const rowHtml=r=>`<button type="button" class="desktop-kpi-row-link" ${r.action?`data-kpi-action="${r.action}"`:"disabled"}><span><strong>${r.name}</strong><small>${r.meta||""}</small></span><b class="${r.amount<0?"negative":""}">${money(r.amount)}</b><i>›</i></button>`;body.innerHTML=`<span class="eyebrow">Финансы</span><h2>${title}</h2><p class="desktop-kpi-period">${subtitle}</p><div class="desktop-kpi-summary">${summary}</div><div class="desktop-kpi-rows">${rows.length?rows.map(rowHtml).join(""):`<div class="desktop-kpi-empty">За этот период записей нет</div>`}</div><div class="desktop-kpi-total"><span>Итого</span><strong class="${total<0?"negative":""}">${money(total)}</strong></div>`;body.querySelectorAll("[data-kpi-action]").forEach(el=>el.onclick=()=>{const [kind,id]=el.dataset.kpiAction.split(":");modal.hidden=true;if(kind==="expense")openExpenseDialog("",id);else if(kind==="payment")openPaymentDialog("",id);else if(kind==="car")openCar(id,"finance");else if(kind==="page")showPage(id)});modal.hidden=false
}
