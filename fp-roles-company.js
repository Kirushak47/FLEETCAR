...
async function openAdminDriverProfile(userId){
 ...
 vehicle.innerHTML=c?`<button type="button" class="driver-profile-vehicle-card" data-profile-car="${c.id}"><strong>${model(c).brand} ${model(c).model}</strong><span>${c.plate||"Без номера"} · ${c.city||"Без города"}</span><em>${window.currentAssignmentAcceptedLocally?.(c)||c.driverAcceptedAt?"Автомобиль принят":"Назначен, ожидает приёмки"}</em></button>`:'<div class="owner-empty">Автомобиль не назначен</div>';
