...
(()=>{'use strict';
 ...
 async function refreshCloudFiles(){const el=$('#fileStorageStats');if(!el)return;try{const files=await window.FleetPilot?.Files?.list?.({limit:1000});if(!Array.isArray(files)){el.textContent='Storage ещё не подключён';return}const size=files.reduce((s,x)=>s+Number(x.size_bytes||0),0);el.innerHTML=`<strong>${files.length}</strong> файлов · <strong>${humanSize(size)}</strong><br><small>Доступны из Workspace</small>`}catch(error){el.textContent='Не удалось проверить облачные файлы';console.warn('FleetPilot cloud file stats',error)}}
