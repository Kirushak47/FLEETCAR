...
(()=>{'use strict';
 ...
 async function get(id,{includeDeleted=false}={}){if(!id)return null;const {c,ws}=assertReady();let q=c.from('workspace_files').select('*').eq('workspace_id',ws).eq('id',String(id));if(!includeDeleted)q=q.is('deleted_at',null);const {data,error}=await q.maybeSingle();if(error)throw error;return data||null}
 ...
 async function remove(file){if(!file?.id||!file?.storage_path)throw new Error('Файл не найден');const {c}=assertReady();const now=new Date().toISOString();const {error:metaError}=await c.from('workspace_files').update({deleted_at:now}).eq('workspace_id',workspaceId()).eq('id',file.id);if(metaError)throw metaError;const {error:storageError}=await c.storage.from(BUCKET).remove([file.storage_path]);if(storageError)console.warn('FleetPilot storage remove',storageError);window.dispatchEvent(new CustomEvent('fleetpilot:files-changed',{detail:{action:'delete',file}}));return true}
 async function replace(fileRecord,newFile,opts={}){const uploaded=await upload(newFile,{entityType:opts.entityType||fileRecord.entity_type,entityId:opts.entityId||fileRecord.entity_id,category:opts.category||fileRecord.category});await remove(fileRecord);return uploaded}
 FP.Files={bucket:BUCKET,upload,list,get,signedUrl,open,download,remove,replace,workspaceId};
