...
(()=>{'use strict';
 ...
 function installDefinition(){
  ...
  if(!group.some(x=>x?.[0]===permission))group.push([permission,'Управлять водителями и создавать водителей']);
 }
 function controls(){
 ...
 function apply(){installDefinition();const ok=allowed();controls().forEach(el=>{el.hidden=!ok;el.setAttribute('aria-hidden',String(!ok));if('disabled'in el)el.disabled=!ok});if(!ok)dialogs().forEach(d=>{if(d.open&&/создать|добавить|новый/i.test(d.textContent||''))d.close?.()});try{window.renderRolePermissions?.()}catch{}}
 ...
 ['fleetpilot:access-ready','fleetpilot:permissions-changed','fleetpilot:modules-ready'].forEach(ev=>window.addEventListener(ev,()=>setTimeout(apply,0)));
 document.addEventListener('DOMContentLoaded',()=>setTimeout(apply,0));
 const mo=new MutationObserver(()=>apply());document.addEventListener('DOMContentLoaded',()=>mo.observe(document.body,{subtree:true,childList:true}),{once:true});
