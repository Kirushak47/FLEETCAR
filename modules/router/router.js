...
(()=>{
 ...
 const defaultPage=()=>{
  try{if(window.enterpriseCurrentRole?.()==='driver')return'driverPortalPage'}catch{}
  return ['fleetPage','driversPage','paymentsPage','expensesPage','repairsPage','documentsPage','analyticsPage','calendarPage'].find(accessible)||'fleetPage';
 };
 ...
 const pageForRoute=route=>{
  const parts=clean(route).split('/').filter(Boolean),root=parts[0];
  if(root==='car')return{page:'carPage',parts};
  return{page:PAGES[root]||null,parts};
 };
 function apply(route=current()||remembered(),{replace=false}={}){
  route=clean(route);
  if(!route){const page=defaultPage();route=routeForPage(page)}
  const parsed=pageForRoute(route);
  if(parsed.page==='carPage'){
   const id=parsed.parts[1],tab=parsed.parts[2]||'info';
   const target=typeof window.fleetPilotFindCar==='function'?window.fleetPilotFindCar(id):null;
   if(target){window.openCar?.(target.id,tab);remember(route);return true}
   route='fleet';
  }
  let page=pageForRoute(route).page;
  if(!page||!document.getElementById(page)||!accessible(page)){page=defaultPage();route=routeForPage(page)}
  remember(route);
  const wanted=hash(route);
 ...
 function navigatePage(page,{replace=false}={}){return apply(routeForPage(page),{replace})}
 ...
 window.addEventListener('fleetpilot:access-ready',()=>requestAnimationFrame(()=>apply(current()||remembered(),{replace:true})));
